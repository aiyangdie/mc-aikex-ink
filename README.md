...
鼠标锁定兼容：若浏览器拒绝 Pointer Lock，自动使用 WASD 移动、画布内直接移动鼠标换视角、左键攻击的兼容模式；右键仍可放置，Esc 暂停，失焦清空按键。内置浏览器已实测移动、AK 显示、暂停和继续。
...
进入联机房后按 **T** 打开聊天，Enter 发送，Esc 关闭。输入完全匹配且区分大小写的 `Maydaymayday` 会触发本房间核弹：施放者免疫，其他玩家和生灵死亡，当前维度形成半径 12 方块的弹坑；每房间 5 分钟冷却。Boss 死亡后 60 秒在本房间满血刷新，单机亦然。房主可以点击聊天 HUD 中的“重置地形”，确认后清除房间所有维度的建设和弹坑，不影响背包与 Boss 倒计时。以上操作会改变房间状态，务必在自己的测试房间使用。
