...
test('nuke spares caster, kills all other room players and creatures and Boss',()=>{
 ...
 assert.equal(resolveNuke(room,caster,10001),null);
 assert.equal(resolveNuke(room,caster,310000).edits.length>0,true);
});
test('crater bounded and capacity failure causes no partial deaths or cooldown',()=>{
 ...
 assert.equal(resolveNuke(room,caster,10000),null);
 assert.equal(victim.hp,20);assert.equal(room.boss.hp,1500);assert.equal(terrain.size,0);
 assert.equal(room.lastNukeAt,0);
});
test('dead and non-room caster cannot launch',()=>{
 const {caster,room}=fixture();assert.equal(resolveNuke(room,peer('fake'),10000),null);
 caster.hp=0;assert.equal(resolveNuke(room,caster,10000),null);
});
